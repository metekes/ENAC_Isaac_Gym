...
class AerialRobotWithObstaclesCfg(BaseConfig):
    ...
    class env:
        ...
        num_actions = 4
        env_spacing =  7.0  # not used with heightfields/trimeshes
        episode_length_s = 10 # episode length in seconds
        num_control_steps_per_env_step = 10 # number of control & physics steps between camera renders
        onboard_camera_hor_fov = 87.0 # horizontal fov of the onboard camera
        enable_onboard_cameras = True # enable onboard cameras
        enable_save_onboard_cameras = False # enable to save the images of the onboard camera
        reset_on_collision = True # reset environment when contact force on quadrotor is above a threshold
        reset_on_land = True # reset environment when height is below a threshold
        reset_on_high_euler_angels = True # reset the environment if the roll and pitch angels are too high
    ...
    class object_asset_params(asset_state_params):
        ...
        specified_position = [-1000.0, -1000.0, 0.5] # if > -900, use this value instead of randomizing the ratios
