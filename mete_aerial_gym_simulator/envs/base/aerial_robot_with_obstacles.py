...
from envs.base.aerial_robot_with_obstacles_config import AerialRobotWithObstaclesCfg
...
from utils.helpers import asset_class_to_AssetOptions
import time
import cv2
...
class AerialRobotWithObstacles(BaseTask):
    ...
    def __init__(self, cfg: AerialRobotWithObstaclesCfg, sim_params, physics_engine, sim_device, headless):
        ...
        (self.roll, self.pitch, self.yaw) = get_euler_xyz(self.root_quats)

        self.initial_positions = torch.zeros_like(self.root_positions)
        ...
        self.land = torch.zeros(self.num_envs, device=self.device)
        self.land_altitude = 0.15
        self.eulers = torch.zeros(self.num_envs, device=self.device)
        self.roll_threshold = np.deg2rad(80)
        self.pitch_threshold = np.deg2rad(80)
        ...
        self.action_upper_limits = torch.tensor(
            [1.5, 1.5, 1.5, 1.5], device=self.device, dtype=torch.float32) * 0.7
        ...
        if self.cfg.env.enable_onboard_cameras:
            self.full_camera_array = torch.zeros((self.num_envs, 360, 480), device=self.device)
    ...
    def _create_envs(self):
        ...
        # asset_root = '/home/murat/Projects/gym/mete_aerial_gym_simulator/resources/robots/quad'
        ...
        camera_props.horizontal_fov = self.cfg.env.onboard_camera_hor_fov
        ...
        local_transform.r = gymapi.Quat.from_euler_zyx(0.0, np.radians(90.0), 0)
        ...
        for i in range(self.num_envs):
            ...
            for dict_item in env_asset_list:
                ...
                urdf_list = ['1_x_1_wall.urdf', '2_x_2_wall.urdf', 'small_cube.urdf', 'cube.urdf', 'sphere.urdf', 'small_sphere.urdf']
                filename = urdf_list[random.randint(0, 5)]

                # folder_path = '/home/murat/Projects/gym/mete_aerial_gym_simulator/resources/models/environment_assets/objects'
    ...
    def step(self, actions, x, y):
        ...
        self.compute_reward(x, y, self.collisions)
        ...
        if self.cfg.env.reset_on_land:
            self.reset_buf = torch.where(self.land > 0, ones, self.reset_buf)

        if self.cfg.env.reset_on_high_euler_angels:
            self.reset_buf = torch.where(self.eulers > 0, ones, self.reset_buf)
        ...
        # self.root_states[:, 0:3] =  torch.tensor([0, 0, 2], device = self.device)
        # self.gym.set_actor_root_state_tensor(self.sim, self.root_tensor)
        return self.obs_buf, self.privileged_obs_buf, self.rew_buf, self.reset_buf, self.extras, self.images
    ...
    def reset_idx(self, env_ids):
        ...
        drone_positions = ((self.env_upper_bound[env_ids] - self.env_lower_bound[env_ids] -
                           0.50)*drone_pos_rand_sample + (self.env_lower_bound[env_ids]+ 0.25)) * torch.tensor([0.5, 0.5, 1.2], dtype=torch.float32, device=self.device)
        ...
        self.root_states[env_ids,
                         0:3] = drone_positions
        self.root_states[env_ids,
                         2] = torch.tensor([5], dtype=torch.float32, device='cuda:0')
        self.root_states[env_ids,
    ...
    def check_collisions(self):
        ...
        # if self.collisions[0] == 1: print(11111111111111111111111111111111111111111111111111111111111111111111111111)
    
    def check_land(self):
        ones = torch.ones((self.num_envs), device=self.device)
        zeros = torch.zeros((self.num_envs), device=self.device)
        self.land[:] = 0
        self.land = torch.where(self.root_positions[:, 2] < self.land_altitude, ones, zeros)

    def check_eulers(self):
        ones = torch.ones((self.num_envs), device=self.device)
        zeros = torch.zeros((self.num_envs), device=self.device)
        self.eulers[:] = 0
        (roll, pitch, _) = get_euler_xyz(self.root_quats)
        roll = torch.where(roll > np.pi, 2*np.pi - roll, roll)
        pitch = torch.where(pitch > np.pi, 2*np.pi - pitch, pitch)
        self.eulers = torch.where(torch.logical_or(torch.abs(roll) > self.roll_threshold, torch.abs(pitch) > self.pitch_threshold), ones, zeros)
        
    def save_images2list(self):
        if self.cfg.env.enable_save_onboard_cameras:
            for env_id in range(self.num_envs):
                # if env_id % 2 == 1 or self.root_positions[env_id, 2] < 3 or abs(self.roll[env_id]) > np.pi/12 or abs(self.pitch[env_id]) > np.pi/12  or self.collisions[env_id] == 1: continue
                self.images.append(self.camera_tensors[env_id].detach().cpu().numpy())
        else:
            for env_id in range(self.num_envs):
                self.images.append(self.camera_tensors[env_id].detach().cpu().numpy())
    ...
    def compute_reward(self, x, y, collisions):
        self.rew_buf[:], self.reset_buf[:], self.initial_positions = compute_quadcopter_reward(
            self.root_positions,
            self.root_quats,
            self.root_linvels,
            self.root_angvels,
            ...
            self.land_altitude,
            (self.roll_threshold, self.pitch_threshold)
        )
...
@torch.jit.script
def compute_quadcopter_reward(root_positions, root_quats, root_linvels, root_angvels, reset_buf, progress_buf, max_episode_length, x, y, collision, initial_positions, land_altitude, attitude_threshold):
    # type: (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, float, Tensor, Tensor, Tensor, Tensor, Tensor, Tuple[float, float]) -> Tuple[Tensor, Tensor, Tensor]

    # excess euler angles
    (roll, pitch, _) = get_euler_xyz(root_quats)
    attitude_cost = (roll%(2*torch.pi) + pitch%(2*torch.pi)) / math.pi
    roll = torch.where(roll > np.pi, 2*np.pi - roll, roll)
    pitch = torch.where(pitch > np.pi, 2*np.pi - pitch, pitch)

    (roll_threshold, pitch_threshold) = attitude_threshold
    attitude_cost = torch.where(torch.logical_or(torch.abs(roll) > roll_threshold, torch.abs(pitch) > pitch_threshold), 200, 0)

    # distance to inital position
    initial_positions = torch.where(progress_buf.expand_as(root_positions.T) < 2, root_positions.T, initial_positions.T)
    initial_positions = initial_positions.T

    initial_dist = torch.sqrt((initial_positions[..., 0] - root_positions[..., 0])**2 +
                             (initial_positions[..., 0] - root_positions[..., 1])**2)
    # initial_reward = 10.0 / (1.0 + initial_dist**2)


    # distance to target
    target_dist = torch.sqrt((x - root_positions[..., 0])**2 +
                             (y - root_positions[..., 1])**2 +
                             ((root_positions[..., 2]))**2 )
    pos_reward = 2.0 / (1.0 + target_dist**2)
    # pos_reward = torch.where(root_positions[..., 2] < 0.2, pos_reward / 5, pos_reward)
    projected_pos_cost = (x - root_positions[..., 0])**2 + (y - root_positions[..., 1])**2

    # uprightness
    ups = quat_axis(root_quats, 2)
    tiltage = torch.abs(1 - ups[..., 2])
    up_reward = 1 / (1.0 + tiltage * tiltage)
    ...
    # z velocity
    z_velocity = abs(torch.div(root_linvels[..., 2], root_positions[..., 2]))
    z_velocity_cost = torch.where(root_positions[..., 2] > 1, 0, z_velocity) * 2

    # x and y velocity
    xy_velocity = abs(torch.div(root_linvels[..., 0], root_positions[..., 2])) + abs(torch.div(root_linvels[..., 1], root_positions[..., 2]))
    xy_velocity_cost = torch.where(root_positions[..., 2] > 1, 0, xy_velocity) * 50

    # number of steps
    num_step = progress_buf * 0.5

    # land successfully
    land = torch.where(root_positions[..., 2] < land_altitude, 1, 0) * 500
    land = torch.where(progress_buf <= 1, 0, land)

    '''
    print('pos_reward:', pos_reward)
    print('spinnage_reward:', spinnage_reward)
    print('z_velocity:', z_velocity)
    print( - (roll + pitch) / math.pi)
    '''

    # combined reward
    # uprigness and spinning only matter when close to the target
    reward = pos_reward + pos_reward * (up_reward + spinnage_reward) - (10000 * collision ) / 10

    reward = torch.where(torch.logical_and(collision == 1, progress_buf <=1), 0, reward)
    ...
    return reward, reset, initial_positions
